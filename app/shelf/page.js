...
import { MapPin, ChevronDown, BadgeCheck, AlarmClock, Pencil } from 'lucide-react';
...
function ListingCard({ _id, itemName, itemPrice, itemStatus, image, category, currentUser, ownerId, showConfirmDelete, setShowConfirmDelete, showEdit, setShowEdit, onDelete, deletingItem, editedStatus, setEditedStatus, onEdit, editingItem, currentItemId, setCurrentItemId }) {
  ...
  return (
    ...
    <div className="bg-white rounded-xl shadow-md hover:shadow-lg inline-block transition-all duration-300 overflow-hidden">
      ...
      <div className="p-4">
        ...
        <h3 className="font-semibold text-lg text-gray-800 mb-1">{itemName}</h3>
        <p className="text-xl font-bold text-indigo-600 mb-2">₹{itemPrice}</p>
        <div className="flex items-center text-sm text-gray-500">
          {itemStatus === "available" && <><BadgeCheck className="h-5 w-5 text-green-500 mr-2" /><span className="text-green-500">Available</span></>}
          {itemStatus === "sold" && <><TriangleAlert className="h-5 w-5 text-red-500 mr-2"/><span className="text-red-500">Sold</span></>}
          {itemStatus === "requested" && <><AlarmClock className="h-5 w-5 text-yellow-500 mr-2"/><span className="text-yellow-500">Requested</span></>}
        </div>
...
function Shelf() {
  ...
  const handleAddItem = async (e) => {
    ...
    if (!formData.imageBase64 || !formData.itemName || !formData.itemPrice || !formData.category || !formData.circle || !formData.circleIdString) {
  ...
  const handleDeleteItem = async (id) => {
    ...
    if (!id) {
      ...
      try {
        const detetedBulletin = await axios.delete('/api/shelf', { data: { id } });
...
const fetchShelf = async () => {
  try {
    ...
    if (res.status === 200) {
      ...
      const filteredItems = temp.filter(item => allowedIds.has(item.circleIdString));
      ...
      <div className="space-y-8">
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/30 backdrop-blur-sm transition-all">
            <form onSubmit={handleAddItem} className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-6 relative animate-fadeIn border border-gray-200">
              <button
