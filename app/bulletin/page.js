  ...
  return (
    <>
      <div className="bg-white p-6 rounded-xl shadow-sm flex flex-col justify-between h-full">
        <div
          className={`${currentUser === ownerId ? "flex flex-row justify-between items-center" : ""}  mb-2 `}
        >
  ...
  const fetchBulletin = async () => {
    try {
      ...
      if (res.status === 200) {
        ...
        const filteredBulletins = temp.filter((item) =>
            ...
            <div className="fixed inset-0 flex items-center justify-center bg-black/10 backdrop-blur-sm z-50">
              <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full relative animate-fadeIn">
                ...
                <Datetime
                  value={
                    newEvent.date
                      ? moment(newEvent.date, "MMMM D, YYYY • h:mm A")
                      : null
                  }
                  onChange={(date) => {
                  ...
                  inputProps={{
                    ...
                    className:
                      "w-full mb-3 px-4 py-2 border rounded-xl text-sm placeholder:text-gray-400",
                  }}
                />
